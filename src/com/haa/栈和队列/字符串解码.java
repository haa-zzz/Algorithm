...
public class 字符串解码 {
    ...
    public String decodeString(String s) {
        ...
        int number = 0;
        Deque<Integer> stackNumber = new ArrayDeque<>();       //数字栈
        Deque<String> stackRes = new ArrayDeque<>();           //字符串栈
        for(Character c : s.toCharArray()){
            if(c == '['){
                stackNumber.push(number);
                stackRes.push(res.toString());             //先把上次拼接好的字符串和前面的乘数入栈做记录
                number = 0;                                 //清零用于下次
                res = new StringBuilder();
            }else if (c == ']'){
                StringBuilder tmp = new StringBuilder();
                int multi = stackNumber.pop();
